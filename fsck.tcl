...
proc getword {inputstring index} {
  ...
  set params 0; set instr 1; incr index; catch {incr end;}
...
# tree:
#  Calls itself recursively and returns the folder content in a tree structure.
#  LEAVE THE prefix PARAMETER BLANK! Call it only with the location parameter,
#  i.e. [tree "/home/perplexa/.irssi/"]
...
# strtol: Converts a string to an unsigned digit.
proc strtoul {string} {
...
# Sends the G-line reason to the logfile if the bot gets banned from a server.
...
proc flood {nick host hand type chan} {
  ...
  if {[matchattr $hand "fmno|fmno" $chan] && $NOFLOOD_FOR_FRIENDS} {
    return 1;
  }
  return 0;
}
